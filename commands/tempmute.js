...
msg.channel.send(``)
  ...
  .replace(`m`," Minute")
  .replace(`w`," Week")
  msg.channel.send(`${user} is a Channel Muted for , ${mutetime}!`)
db.set(`cmuted_${msg.guild.id + user.id}`, 'muteli')
db.set(`ctime_${msg.mentions.users.first().id + msg.guild.id}`, mutetime)
                        
  const muteembed = new Discord.RichEmbed()
        .setTitle('Penal: Channel Mute')
...
msg.guild.channels.get(log).sendEmbed(muteembed)
  ...
  setTimeout(function(){
db.delete(`cmuted_${msg.guild.id + user.id}`)
